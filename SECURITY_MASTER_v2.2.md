# SECURITY MASTER KNOWLEDGE BASE v2.2
# Flutter × Firebase × Supabase × Next.js 모바일/웹 앱 보안 — 범용 참조 파일
# PART 1~12: 범용 — 프로젝트 무관하게 재사용 가능
# PART 13:   프로젝트별 작성 공간 (현장 진단 B2B 앱 예시 포함 — 신규 프로젝트 시 내용 교체)
# PART 14~18: 범용 — 2026 글로벌 위협 인텔리전스 반영 신규 섹션
#
...
# │  이 파일 섹션         참조 대상                   연결 규칙              │
# │  ─────────────────────────────────────────────────────────────────────── │
# │  PART 3 인증·토큰     CLAUDE.md [Auth Config]     프로바이더·토큰 만료   │
# │                                                   등 프로젝트별 값은     │
# │                                                   CLAUDE.md가 정의      │
# │  PART 12 RBAC         CLAUDE.md [Roles]           역할명·권한 목록은     │
# │                                                   CLAUDE.md, 구현 패턴은 │
# │                                                   이 파일               │
# │  PART 11 데이터거버넌스 CLAUDE.md [Data Class.]   분류 등급·필드 매핑은  │
# │                                                   CLAUDE.md, 처리 규칙은 │
# │                                                   이 파일               │
# │  PART 5 UI보안·10 체크  DESIGN_MASTER §터치타겟   터치44dp/전문직56dp,  │
# │                         §입력필드·§에러상태       에러상태 디자인은      │
# │                                                   DESIGN_MASTER         │
# │  PART 13-4 오프라인     DESIGN_MASTER §오프라인UX  오프라인 UX 플로우는  │
# │                                                   DESIGN_MASTER, 데이터  │
# │                                                   보호는 이 파일        │
# │  PART 15 AI에이전트     CLAUDE.md [AI Config]     API 키명·모델·MCP     │
# │                                                   서버 목록은 CLAUDE.md  │
...
# │           + PART 18(Supabase — 사용 시)                              │
# │           + PART 19(Next.js 웹앱 — 해당 시)                         │
# │  Phase 2: + PART 3(인증+세션) + PART 5(WebView+UGC — 사용 시)       │
# │           + PART 9-2(Crashlytics PII 방지)                          │
# │           + PART 14(결제 — 사용 시)                                  │
# │  Phase 3: + PART 11(거버넌스) + PART 12(RBAC+모니터링+TTL)          │
# │           + PART 13-6(AI API — 사용 시) + PART 15(Agentic AI — 사용 시)│
# │           + PART 16(인시던트 대응 기본)                              │
# │           + PART 7-9(App Check 서버검증) + PART 7-11(Race Condition) │
# │           + PART 12-5(계정 탈취 사후 대응)                          │
# │  Phase 4: 전체 PART 확인 + PART 10 배포 전 전수 검사                │
# │           + PART 3-7(Passkey 도입 검토)                             │
# │           + PART 4-1(SPKI Pinning — 금융/의료 앱)                   │
# │           + PART 4-5(PQC Crypto-Agility 코드 구조 확보)             │
# │           + PART 8-3(시크릿 로테이션 정책 수립)                      │
# │           + PART 15(Agentic AI 전체 — AI 연동 시)                   │
# │           + PART 17(규제 컴플라이언스 전체 검토)                     │
# │           + PART 6-6(문자열보호) + PART 6-7(물리탈취) 최종 확인     │
# │  ※ PART 1(위협 모델)은 프로젝트 설정 시 1회 읽으면 충분             │
# └──────────────────────────────────────────────────────────────────────┘
#
# ┌─ 목차 (TOC) ─────────────────────────────────────────────────────────┐
# │  §0      상호참조 맵 (Cross-Reference Map) — 파일 헤더에 포함       │
# │  PART 1   위협 모델 — OWASP Mobile Top 10 (2024)                     │
# │  PART 2   데이터 보안 (2-1~2-6)                                      │
# │  PART 3   인증 & 인가 (3-1~3-7)                                      │
# │  PART 4   네트워크 보안 — SSL/TLS & Pinning (4-1~4-5)               │
# │  PART 5   앱 내부 보안 — WebView & 딥링크 & UGC (5-1~5-3)           │
# │  PART 6   앱 무결성 보호 (6-1~6-7) ← 6-6 문자열보호·6-7 물리탈취 신설│
# │  PART 7   Firebase 보안 (7-1~7-11) ← 7-9 App Check서버·7-10 FCM    │
# │           · 7-11 Race Condition 신설 ※ CF Gen1 문법 기준            │
# │  PART 8   공급망 보안 (8-1~8-6) ← 8-6 SBOM 신설                    │
# │  PART 9   개인정보 보호 (9-1~9-4)                                    │
# │  PART 10  보안 체크리스트 — 배포 전 전수 검사 + Phase 게이트         │
# │  PART 11  데이터 거버넌스 & 소유권 (11-1~11-5)                       │
# │           ← 11-5 HNDL(Harvest Now, Decrypt Later) 대응 신설         │
# │  PART 12  RBAC 설계 (12-1~12-5) ← 12-5 계정탈취 사후대응 신설      │
# │  PART 13  도메인 특화 보안 (13-0~13-7)                               │
# │  ──── 이하 v2.0 신규 ────                                           │
# │  PART 14  결제 · 구독 보안 ★NEW                                     │
# │  PART 15  AI 에이전트 보안 — OWASP Agentic AI Top 10 (2026) ★NEW    │
# │  PART 16  인시던트 대응 ★NEW                                         │
# │  PART 17  규제 · 컴플라이언스 매핑 ★NEW                             │
# │  PART 18  Supabase 보안 ★NEW                                        │
# │  ──── 이하 v2.2 신규 ────                                           │
# │  PART 19  Next.js · 웹앱 보안 (19-1~19-6) ★NEW                     │
# │           ← CSP·CSRF·HTTPS헤더·환경변수·오픈리다이렉트·Rate Limit   │
# │  부록     보안 도구 & 리소스 + 용어 정의 + CHANGELOG 포맷 템플릿     │
# └──────────────────────────────────────────────────────────────────────┘
#
# ┌─ ★ SOURCE OF TRUTH 선언 ★ ─────────────────────────────────────────────┐
# │  보안·RBAC·CLIENT_VIEW·데이터 거버넌스에 관한 모든 규칙의 최종 기준은    │
# │  이 파일(SECURITY_MASTER_v2.2.md)이다.                                   │
# │                                                                          │
...
# │  PART 3  인증 & 인가    — 기본 인증 + OAuth PKCE + 세션 관리 + Passkey │
# │  PART 4  네트워크 보안  — SSL/TLS + SPKI/Cert Pinning + HTTP/3 + PQC │
# │  PART 5  앱 내부 보안   — WebView + 딥링크/Intent + UGC XSS 방어     │
# │  PART 6  앱 무결성      — 난독화 + 루트감지 + 스크린샷 방지 + 강제업데이트│
# │          + 문자열보호(6-6) + 물리기기탈취(6-7) ★v2.2               │
# │  PART 7  Firebase 보안  — Rules + Auth + App Check + CF 보안        │
# │          + App Check 서버검증(7-9) + FCM(7-10) + Race Condition(7-11) ★v2.2│
# │  PART 8  공급망 보안    — 의존성 + 빌드 + 서명 + CI/CD + SBOM      │
# │  PART 9  개인정보 보호  — GDPR/PIPA + Crashlytics PII + 계정 삭제 │
# │  PART 10 보안 체크리스트 — 배포 전 전수 검사 + Phase 게이트 (통합)  │
# │  PART 11 데이터 거버넌스 — 데이터 분류/소유권/보존기간 + HNDL 대응 │
# │  PART 12 RBAC 설계      — 역할 기반 접근 제어 + 모니터링 + 탈취대응 ★v2.2│
# │  PART 13 도메인 특화 보안 — 프로젝트별 작성 (템플릿 제공) + AI API  │
# │  PART 14 결제·구독 보안  — PCI DSS + IAP 영수증 + 구독 관리 ★NEW   │
# │  PART 15 AI 에이전트 보안 — Agentic AI + MCP + Shadow AI ★NEW      │
# │  PART 16 인시던트 대응   — 탐지→격리→분석→복구→학습 ★NEW            │
# │  PART 17 규제 매핑       — 한국/글로벌/2026 신규 규제 ★NEW          │
# │  PART 18 Supabase 보안   — RLS + Auth + Edge Functions ★NEW        │
# │  PART 19 Next.js·웹앱 보안 — CSP + CSRF + 헤더 + 환경변수 ★v2.2   │
# │  부록    보안 도구 & 리소스 + 용어 정의                              │
# └──────────────────────────────────────────────────────────────────────┘
#
# 준거: OWASP Mobile Top 10 (2024) · OWASP MASVS v2.0 · MASTG
#       OWASP Agentic AI Top 10 (2026) ★NEW
#       OWASP LLM Top 10 (2025) · NIST SP 800-63B (Rev.4 포함) · NIST AI RMF
#       FIDO2/WebAuthn (W3C) · Passkeys (FIDO Alliance)
#       CrowdStrike 2026 GTR · Gartner Cybersecurity Trends 2026 ★NEW
#       Cisco State of AI Security 2026 ★NEW
#       GDPR · PIPA · EU AI Act · NIS2 · DORA · CMMC 2.0 ★NEW
#       Google Android Security · Google Play Data Deletion Policy
...
#       NIST SP 800-218 SSDF v1.2 (2025.12) ★v2.1 추가
#       OWASP MASWE (Mobile App Security Weakness Enumeration) ★v2.1 추가
#       OWASP CycloneDX (SBOM 표준) ★v2.1 추가
#       EU Cyber Resilience Act (CRA) ★v2.1 추가
#       OWASP API Security Top 10 · W3C CSP Level 3 ★v2.2 추가
...
>   MASVS-STORAGE    → PART 2 (데이터 보안)
>   MASVS-CRYPTO     → PART 4 (네트워크/암호화)
>   MASVS-AUTH       → PART 3 (인증/인가)
>   MASVS-NETWORK    → PART 4 (네트워크 보안)
>   MASVS-PLATFORM   → PART 5 (WebView/딥링크)
>   MASVS-CODE       → PART 6 (앱 무결성) + PART 8 (공급망)
>   MASVS-RESILIENCE → PART 6 (앱 무결성)
>   MASVS-PRIVACY    → PART 9 (개인정보)
...
### 왜 데이터 보안이 최우선인가 — 2025/2026 통계 ★v2.1
...
### 3-5. 인증 에러 메시지 — 계정 열거(Enumeration) 공격 방지
...
> ★ **에러 메시지 UX 표현(사용자에게 보이는 문구 설계)** → DESIGN_MASTER_v3.2.md PART 3-3 참조
> 이 섹션(보안)은 어떤 정보를 노출하면 안 되는지 정의하고,
> PART 3-3(UX)은 그 제약 안에서 인간 언어로 에러를 표현하는 방법을 정의한다.
...
### 4-3. Apple ATS (App Transport Security)
...
### 4-5. 포스트-양자 암호화 (PQC) — Crypto-Agility 준비
...
### 5-3. 사용자 생성 콘텐츠(UGC) & HTML 렌더링 보안
...
### 6-5. RASP(Runtime Application Self-Protection) — Phase 3+ 필수 ★v2.1
...
RASP 필수 검증 항목:
  ...
  [권장] 환경 이상 탐지 결과를 서버에 보고 (PART 12-4 감사 로그 연동)

Flutter 구현:
  freerasp 패키지 (Talsec) — PART 6-2에서 다루는 패키지
...
**핵심 규칙**
```
[필수] API 키·시크릿 → PART 2-3 레벨 적용 (앱 내 포함 금지)
...
시나리오 1: 잠금 해제된 기기 탈취
  → 앱이 이미 열려있는 상태
  → 방어: 비활동 타임아웃(PART 3-6) + 민감화면 재인증 요구
...
시나리오 4: 앱 백업 추출 (adb backup, iTunes)
  → 로컬 캐시/DB 파일 복사
  → 방어: backup_rules.xml (6-6) + Hive AES-256 (PART 13-4)
```
...
**핵심 규칙**
```
[필수] "모든 기기에서 로그아웃" 기능 — 설정 화면 필수 제공 (PART 3-6 연동)
...
### 7-5. App Check & Play Integrity — 정품 앱 인증
...
### 7-7-1. Firebase 미설정 노출 방어 — APK 역공학 대응 ★v2.1
...
### 7-9. App Check 서버사이드 검증 — 클론 앱 차단 ★v2.2
...
### 7-11. Race Condition & 비즈니스 로직 보안 ★v2.2
...
exports.purchaseItem = functions.https.onCall(async (data, context) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    // 클라이언트 전송 가격 신뢰 금지 → DB 가격 재조회 (PART 14-1 연동)
...
**Idempotency-Key — 중복 요청 방어 (PART 4-4 연동)**
```javascript
...
보관 원칙:
  ...
  ✅ 보관 기간: 해당 버전이 스토어에서 활성인 동안 + 6개월
     → 구버전 강제 업데이트(PART 6-4) 적용 후 6개월이면 삭제 가능
...
PR 생성 시 자동 실행:
  ...
  4. custom lint rules — 프로젝트 보안 규칙 (PART 2 금지 패턴 등)
...
> SSL 핀 로테이션은 PART 4-1에서 다뤘지만, Cloud Functions 환경변수에 저장된
...
서명 키 (Android Keystore):         교체 불가 — 분실 방지만 (PART 8-2)
...
### 8-4. SDK Privacy Manifests (Apple 필수)
...
### 8-6. SBOM (Software Bill of Materials) — 소프트웨어 자재명세서
*[NTIA SBOM Minimum Elements, EU CRA(Cyber Resilience Act), CISA SBOM Guidance 2025+]*

> ★ v2.0 신규: 공급망 공격이 2025년 3배 증가(Group-IB 2026 보고서).
...
exports.deleteUserAccount = functions
  ...
  // ── 1. 법적 보존 의무 데이터 확인 (PART 11-3 연계) ──────────
...
**클라이언트 — 삭제 요청 플로우**
```dart
// ✅ 삭제 전 재인증 필수 (민감 작업 보호 — PART 3 참조)
...
✅ 법적 보존 데이터는 익명화 후 보존 기간까지 유지 (PART 11-3)
✅ 삭제 완료 확인 이메일/알림 발송
...
### 9-4. App Privacy Labels & Data Safety 작성 가이드
...
주의사항:
  ✅ "데이터가 전송 중 암호화됨" → HTTPS 적용 확인 (PART 4)
  ✅ "사용자가 데이터 삭제를 요청할 수 있음" → 계정 삭제 구현 필수 (PART 9-3)
...
- [ ] `[선택]` SPKI/Certificate Pinning 적용 (금융/의료/결제 앱 권장 — PART 4-1)
- [ ] `[공통]` 클립보드 자동 클리어 30초 적용
- [ ] `[공통]` 키보드 캐시 비활성화 (민감 필드)
- [ ] `[공통]` Crashlytics Custom Keys/Logs에 PII 미포함 확인 (PART 9-2)
...
- [ ] `[공통]` Sign in with Apple 포함됨 (소셜 로그인 1개 이상 시 필수 — PART 3-3)
- [ ] `[공통]` 인증 에러 통합 메시지 적용 — 계정 열거 방지 (PART 3-5)
- [ ] `[공통]` 생체인증 fallback 처리됨
- [ ] `[공통]` 세션 최대 수 정책 적용됨 (PART 3-6)
...
### [ 네트워크 ]
- [ ] `[공통]` TLS 1.2 미만 차단
- [ ] `[선택]` 인증서 핀닝 — 네이티브 SPKI 권장 (금융/의료/결제 앱 — PART 4-1)
- [ ] `[선택]` 핀 교체 메커니즘 구현됨 (인증서 핀닝 적용 시 필수)
- [ ] `[선택]` Idempotency-Key 헤더 적용 — 결제/계정생성 등 중요 POST 요청 (0-RTT Replay 방어 — PART 4-4)
- [ ] `[Phase4+]` Crypto-Agility 구조 확보 — 암호화 알고리즘 상수화 + CryptoService 집중화 (PART 4-5)
- [ ] `[Phase4+]` AES-256 이상 사용 확인 (AES-128 사용 없음 — 양자 시대 안전 마진 감소 방지)

### [ WebView & 딥링크 & UGC ]
- [ ] `[공통]` JavaScript Bridge 화이트리스트 적용 (WebView 사용 시)
- [ ] `[공통]` 딥링크 파라미터 검증됨
- [ ] `[UGC]` 사용자 생성 HTML 콘텐츠 서버사이드 새니타이징 적용 (PART 5-3)
- [ ] `[UGC]` flutter_html 등 HTML 렌더링 위젯 — javascript: scheme 차단 (PART 5-3)
- [ ] `[UGC]` 링크 클릭 시 HTTPS 화이트리스트만 허용 (PART 5-3)

### [ 앱 무결성 ]
- [ ] `[공통]` Dart `--obfuscate` + `--split-debug-info` 활성화
- [ ] `[공통]` Android ProGuard/R8 `minifyEnabled true` 활성화
- [ ] `[공통]` 루트/탈옥 감지 + 경고 처리
- [ ] `[공통]` 스크린샷 방지 — Android FLAG_SECURE + iOS 캡처 감지 (PART 6-3)
- [ ] `[iOS]` 백그라운드 스냅샷 블러 처리 — AppDelegate + Flutter 이중 적용 (PART 6-3)
- [ ] `[공통]` Firebase App Check 활성화 (Firebase 사용 시)
- [ ] `[공통]` 강제 업데이트 메커니즘 구현됨 (PART 6-4)

### [ Firebase & Cloud Functions ]
- [ ] `[공통]` Firestore Rules 자동화 테스트 작성됨 (PART 7-7)
- [ ] `[공통]` Cloud Functions onCall 함수 — 인증 확인 적용됨
- [ ] `[공통]` Cloud Functions onRequest 함수 — CORS 화이트리스트 적용됨 (PART 7-8)
- [ ] `[공통]` Cloud Functions 입력값 — Zod/Joi 스키마 검증 적용됨 (PART 7-8)
- [ ] `[공통]` Cloud Functions 리소스 설정 — 함수별 timeout/memory 적정값 (PART 7-8)
...
- [ ] `[공통]` CI/CD 시크릿이 환경변수(Secrets)로만 주입됨 (PART 8-3)
- [ ] `[공통]` API 키 정기 로테이션 주기 정책 수립됨 (PART 8-3)
- [ ] `[공통]` 긴급 키 교체 절차 문서화됨 — Zero-Downtime 보장 (PART 8-3)

### [ 계정 관리 ]
- [ ] `[공통]` 계정/데이터 삭제 기능 구현됨 (PART 9-3, Google Play 필수)
- [ ] `[공통]` 삭제 시 Firestore + Storage + Auth 연쇄 삭제됨
- [ ] `[공통]` 법적 보존 데이터 익명화 처리됨 (삭제 아닌 경우)

### [ 보안 모니터링 ] *(Phase 3+)*
- [ ] `[공통]` 감사 로그 표준 스키마 적용됨 (PART 12-4)
...
- [ ] `[공통]` Firestore TTL 정책 설정됨 — expiresAt 필드 기반 자동 삭제 (PART 12-4-4)
- [ ] `[공통]` 감사 로그 생성 시 심각도별 TTL 자동 설정됨 (PART 12-4-4)

### [ AI API 보안 ] *(AI 연동 앱만)*
- [ ] `[AI연동]` AI API 키 Cloud Functions 프록시 경유 (Level 2+)
- [ ] `[AI연동]` PII 마스킹 후 AI 전송됨
- [ ] `[AI연동]` 프롬프트 인젝션 방어 적용됨 (PART 13-6)
- [ ] `[AI연동]` 사용량 Rate Limit 적용됨 (사용자별/일별)
- [ ] `[AI연동]` AI 데이터 공유 동의 모달 구현됨 (Apple 5.1.2(i))


### [ RASP & 역공학 방어 ] *(Phase 3+ — v2.1 추가)*
- [ ] `[Phase3+]` RASP 전체 검증 활성화 — 디버거·후킹·변조·에뮬레이터 감지 (PART 6-5)
- [ ] `[공통]` Firebase Rules에 'if true' 패턴 없음 자동 검증 (PART 7-7-1)
- [ ] `[공통]` google-services.json API 키에 앱 서명+패키지명 제한 적용 확인 (PART 7-7-1)

### [ AI 코딩 보안 ] *(AI 도구 사용 시 — v2.1 추가)*
- [ ] `[AI연동]` AI 생성 코드 SAST 스캔 적용됨 (PART 15-7)
- [ ] `[AI연동]` AI 코딩 도구 코드 전송 정책 확인됨 (PART 15-8)
- [ ] `[AI연동]` Vibe coding 결과물 보안 리뷰 프로세스 적용됨 (PART 15-8)
- [ ] `[Phase3+]` CycloneDX 포맷 SBOM 생성됨 (PART 8-6)
- [ ] `[공통]` CRITICAL 취약점 패치 24시간 이내 적용 가능 확인 (PART 15-7)

### [ 물리 보안 & 클론 앱 방어 ] *(v2.2 추가)*
- [ ] `[공통]` AndroidManifest allowBackup="false" 설정됨 (PART 6-6)
- [ ] `[공통]` backup_rules.xml — DB·키 파일 제외됨 (PART 6-6)
- [ ] `[공통]` "모든 기기에서 로그아웃" 기능 구현됨 — Firebase Auth revokeRefreshTokens() (PART 6-7)
- [ ] `[공통]` 새 기기 첫 로그인 시 보안 알림 발송됨 (PART 12-5)
- [ ] `[Firebase]` App Check 서버사이드 토큰 검증 활성화됨 (PART 7-9)
- [ ] `[Firebase]` FCM 페이로드에 민감 데이터 미포함 확인 (PART 7-10)
- [ ] `[Firebase]` 로그아웃 시 FCM 토큰 삭제됨 (PART 7-10)
- [ ] `[공통]` 내부 URL·테스트 자격증명 프로덕션 빌드에서 제거 확인 (PART 6-6)

### [ 비즈니스 로직 & Race Condition ] *(v2.2 추가)*
- [ ] `[쿠폰/재고]` 한정 리소스 처리 — Firestore Transaction 사용됨 (PART 7-11)
- [ ] `[결제/주문]` Idempotency-Key 적용됨 — 중복 결제 방지 (PART 7-11)
- [ ] `[결제]` 클라이언트 전송 가격 서버 DB 재조회 후 검증됨 (PART 14-1 + 7-11)
- [ ] `[선착순]` 이벤트 참가 중복 방지 Transaction 구현됨 (PART 7-11)

### [ 웹앱 보안 ] *(Next.js 프로젝트 — v2.2 추가)*
- [ ] `[웹앱]` next.config.js 7종 보안 헤더 적용됨 (PART 19-1)
- [ ] `[웹앱]` SameSite=lax + HttpOnly + Secure 쿠키 설정됨 (PART 19-3)
- [ ] `[웹앱]` NEXT_PUBLIC_ 변수에 시크릿 미포함 확인 (PART 19-4)
- [ ] `[웹앱]` .env 파일 .gitignore 포함 확인 (PART 19-4)
- [ ] `[웹앱]` dangerouslySetInnerHTML 사용처 DOMPurify 새니타이징 적용됨 (PART 19-5)
- [ ] `[웹앱]` 리다이렉트 URL 화이트리스트 검증됨 — 오픈 리다이렉트 방어 (PART 19-5)
- [ ] `[웹앱]` API Route 입력값 Zod 스키마 검증 적용됨 (PART 19-6)
- [ ] `[웹앱]` 민감 API Rate Limiting 적용됨 (PART 19-6)
...
- [ ] `[공통]` App Privacy Labels (Apple) 작성 완료 — 실제 수집 데이터와 일치 (PART 9-4)
- [ ] `[공통]` Data Safety Section (Google) 작성 완료 — 실제 수집 데이터와 일치 (PART 9-4)

**Apple 고유**
- [ ] `[iOS]` SDK Privacy Manifests 포함 확인 — Xcode Privacy Report 생성 (PART 8-4)
- [ ] `[iOS]` ATS(App Transport Security) 예외 없음 — 전체 HTTPS 강제 (PART 4-3)
...
async function suspendAccount(uid, reason) {
  ...
  // 4. 관리자 CRITICAL 알림 (PART 12-4 sendAlert 연동)
...
파기 방법:
  탈퇴 사용자: Cloud Functions deleteUserAccount 자동 처리 (PART 9-3)
...
비용 폭탄 방지:
  ...
  ✅ 비용 임계값(80%) 도달 시 관리자 알림 (PART 12-4 연동)
...
async function recordUsage(refs, uid, inputTokens, outputTokens, costUsd) {
  ...
  // 월간 비용 80% 도달 시 관리자 알림 (PART 12-4 sendAlert 재사용)
...
exports.aiChat = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    // 2. AI API 호출 (PART 13-6-2 프롬프트 보안 적용)
...
PHASE 1 — [코어 기능] — 최소 보안 구축
  ...
  ✅ HTTPS + 입력값 기본 검증
  ✅ 인증 에러 통합 메시지 적용 (PART 3-5)

PHASE 2 — [확장 기능] — 데이터 보안 강화
  ✅ 파일 업로드 보안 (13-3)
  ✅ 오프라인 암호화 저장 (13-4)
  ✅ Firestore Rules 역할별 세분화
  ✅ 세션 관리 정책 구현 (PART 3-6)
  ✅ Firestore Rules 자동화 테스트 작성 (PART 7-7)
  ✅ Cloud Functions 입력 검증 + CORS 적용 (PART 7-8)
  ✅ UGC HTML 렌더링 보안 — 서버사이드 새니타이징 (PART 5-3, UGC 사용 시)
  ✅ Crashlytics PII 유출 방지 적용 (PART 9-2)

PHASE 3 — [수익화] — 결제 보안 추가
  ✅ 결제 서비스 Webhook 검증 (13-5)
  ✅ 법적 문서 불변성 적용 (13-1, 13-2)
  ✅ 감사 로그 활성화 (PART 12-3)
  ✅ 보안 이벤트 모니터링 구축 (PART 12-4)
  ✅ Firestore TTL 정책 설정 — 감사 로그 자동 정리 (PART 12-4-4)
  ✅ AI API 보안 + Rate Limit 적용 (13-6 — AI 연동 시)

PHASE 4 — [고급 기능] — 전체 보안 완성 + 스토어 배포
  ✅ SPKI Pinning + 로테이션 자동화 적용 (PART 4-1, 금융/의료/결제 앱)
  ✅ 루트/탈옥 감지 RASP (PART 6)
  ✅ 스크린샷 방지 — Android + iOS 양 플랫폼 (PART 6-3)
  ✅ iOS 백그라운드 스냅샷 블러 처리 (PART 6-3)
  ✅ 강제 업데이트 메커니즘 적용 (PART 6-4)
  ✅ Firebase App Check + Play Integrity 활성화 (PART 7-5)
  ✅ 계정/데이터 삭제 기능 검증 (PART 9-3)
  ✅ App Privacy Labels / Data Safety 작성 (PART 9-4)
  ✅ SDK Privacy Manifests 구성 (PART 8-4)
  ✅ split-debug-info 심볼 파일 아카이브 정책 적용 (PART 8-2)
  ✅ CI/CD 보안 파이프라인 구축 (PART 8-3)
  ✅ 런타임 시크릿 로테이션 정책 수립 + 주기 캘린더 등록 (PART 8-3)
  ✅ Sign in with Apple 포함 확인 (PART 3-3)
  ✅ Passkey/WebAuthn 도입 검토 — Flutter 공식 지원 확인 후 판단 (PART 3-7)
...
*[v2.0 신규 — 기존 PART 13-5 IAP 영수증 검증을 확장하여 독립 PART로 격상]*

> 결제 보안은 프로젝트 유형(B2C/B2B/SaaS)에 무관하게 적용되는 범용 규칙이다.
> PART 13-5의 IAP 영수증 검증은 프로젝트 특화 예시이며, 이 PART는 범용 원칙을 정의한다.
...
[필수] 무료↔유료 전환 시 권한 즉시 동기화 (PART 12 RBAC 연동)
...
*[v2.0 신규 — 보안 전문가 48%가 2026년 최대 위협으로 지목]*

> AI 에이전트는 단순 LLM 호출을 넘어 자율적으로 도구를 실행하고 데이터에 접근하는 시스템이다.
> PART 13-6(AI API 보안)은 LLM API 프록시 수준의 보안이고,
> 이 PART는 에이전트가 자율적으로 행동할 때 발생하는 고유 위험을 다룬다.
> AI API를 사용하지만 에이전트 기능이 없는 앱은 PART 13-6만으로 충분.
...
### 15-3. MCP(Model Context Protocol) 보안
...
[권장] MCP 서버 업데이트 시 7~14일 쿨다운 후 프로덕션 적용 (PART 8-1 의존성 쿨다운 연동)
```
...
[금지] 개인 GenAI 계정에 Level 3+ 데이터 입력 (PART 11 데이터 분류 연동)
...
### 15-8. Vibe Coding / AI 코딩 보안 위험 ★v2.1
...
*[v2.0 신규 — PART 12-4 모니터링과 연계하여 대응 플로우 체계화]*

> 모니터링(PART 12-4)은 "감지"까지만 다룬다. 감지 이후의 대응·복구·학습 과정을 이 PART에서 정의.

### 16-1. 대응 플로우 (5단계)

```
1. 탐지 (Detect)   → 이상 징후 식별 (PART 12-4 모니터링 + 사용자 제보 + 외부 알림)
                      → CRITICAL 이벤트는 5분 이내 알림 (Slack/FCM — PART 12-4-2)
2. 격리 (Contain)   → 영향 범위 제한
                      → 유출된 키 즉시 폐기, 관련 세션 무효화, 에이전트 중단 (PART 15)
                      → 필요 시 서비스 일부 격리 (유지보수 모드 전환)
3. 분석 (Analyze)   → 근본 원인 파악
                      → 감사 로그 분석 (PART 12-4-3 표준 스키마)
                      → 영향 받은 데이터·사용자 식별
                      → SBOM 기반 취약점 영향 범위 파악 (PART 8-6)
4. 복구 (Recover)   → 취약점 패치 + 서비스 복원 + 데이터 무결성 확인
                      → 영향 받은 사용자 고지 (법적 의무 확인 — PART 17)
                      → 백업에서 데이터 복원 (필요 시)
5. 사후조치 (Learn) → 포스트모템 작성 + 재발 방지 조치
                      → 이 파일 업데이트 (새로운 위협 패턴 추가)
                      → PART 10 체크리스트에 관련 항목 추가
```
...
## ▌PART 17. 규제 · 컴플라이언스 매핑 ★NEW
*[v2.0 신규 — 2026 글로벌 규제 환경 반영. 기존 PART 9·11의 GDPR/PIPA를 통합 확장]*
...
────────────────────────────────────────────────────────────────────────────────
한국:
  개인정보보호법(PIPA)  PART 9, 11, 13                     수집 동의, 보존 기간, 삭제 권리
  정보통신망법           PART 4, 5, 7                       전송 암호화, 접근 통제
  전자금융거래법         PART 14                            결제 보안, 이중 인증
  전자상거래법           PART 11-3                          거래 기록 5년 보존
  통신비밀보호법         PART 11-3                          로그 3개월 보존

글로벌:
  GDPR (EU)             PART 9, 11, 16                     72시간 침해 고지, DPO 지정,
                                                            데이터 이식성(Art.20)
  PCI DSS v4.0          PART 14                            카드 정보 미처리, 서버 검증
  SOC 2                 PART 1~6 전반 + 12-4               감사 로그, 접근 통제
  NIST AI RMF           PART 15                            AI 위험 관리 프레임워크

글로벌 (v2.1 추가):

  NIST SSDF v1.2        PART 8, 10, 전체             Secure SDLC 4단계
  (SP 800-218)                                        (Prepare→Protect→Produce→Respond)
                                                      US EO 14028 연방 필수
  EU CRA                PART 8-6, 9, 17               제품 보안 의무, SBOM 필수,
  (Cyber Resilience Act)                              취약점 보고 의무, CE 마킹 연동


2026 신규:
  EU AI Act             PART 15, 17                        AI 위험 등급 분류, 투명성 요구,
                                                            고위험 AI 시스템 등록 의무
  NIS2 (EU)             PART 8, 16                         공급망 보안 강화, 24시간 초기 보고
  DORA (EU 금융)        PART 14, 16                        디지털 운영 복원력, 침투 테스트
  CMMC 2.0 (미 방산)    PART 전체                          보안 성숙도 인증, $200K~수백만 비용
  OWASP Agentic AI      PART 15                            에이전트 보안 Top 10 대응
```
...
→ PART 16 인시던트 대응 플로우의 "복구" 단계에서 보고 의무 확인 필수
```
...
*[v2.0 신규 — Firebase 대안으로 Supabase 사용 시 적용되는 보안 규칙]*
...
> Firebase와 Supabase를 혼용하는 프로젝트는 PART 7 + PART 18 모두 참조.

### 18-1. RLS(Row Level Security) — 필수
...
[RBAC] 역할 기반 RLS 정책 구현 (PART 12 연동)
[멀티테넌트] 테넌트 격리 RLS 정책 구현 (PART 11-4 연동)
[결제] Stripe 웹훅 서명 검증 구현 (PART 14 연동)
```


# │  부록     보안 도구 & 리소스 + CHANGELOG 포맷 템플릿                  │
# └──────────────────────────────────────────────────────────────────────┘
#
# ┌─ ★ SOURCE OF TRUTH 선언 ★ ─────────────────────────────────────────────┐
# │  보안·RBAC·CLIENT_VIEW·데이터 거버넌스에 관한 모든 규칙의 최종 기준은    │
...
# │  PART 4  네트워크 보안  — SSL/TLS + SPKI/Cert Pinning + HTTP/3 + PQC │
# │  PART 5  앱 내부 보안   — WebView + 딥링크/Intent + UGC XSS 방어 │
# │  PART 6  앱 무결성      — 난독화 + 루트감지 + 스크린샷 방지 + 강제 업데이트│
# │  PART 7  Firebase 보안  — Rules + Auth + App Check + CF 보안        │
# │  PART 8  공급망 보안    — 의존성 + 빌드 + 서명 + CI/CD + 시크릿 로테이션│
# │  PART 9  개인정보 보호  — GDPR/PIPA + Crashlytics PII + 계정 삭제 │
# │  PART 10 보안 체크리스트 — 배포 전 전수 검사 (통합)                  │
# │  PART 11 데이터 거버넌스 — 데이터 분류/소유권/보존기간               │
# │  PART 12 RBAC 설계      — 역할 기반 접근 제어 + 모니터링 ★CLIENT_VIEW│
# │  PART 13 도메인 특화 보안 — 프로젝트별 작성 (템플릿 제공) + AI API  │
# │  부록    보안 도구 & 리소스                                          │
# └──────────────────────────────────────────────────────────────────────┘
#
...
## ▌PART 19. Next.js · 웹앱 보안 ★NEW v2.2
*[OWASP Top 10 (2021), OWASP ASVS v4.0, MDN Security Headers, Next.js Docs]*
*[v2.2 신규 — Flutter × Firebase 외 Next.js / 웹앱 프로젝트 적용]*
...
### 19-2. CSP (Content Security Policy) 상세
...
[권장] CI/CD에서 Gitleaks 실행 — 시크릿 커밋 자동 감지 (PART 8-3 연동)
```

---

### 19-5. XSS / 오픈 리다이렉트 / Clickjacking
...
const CreateOrderSchema = z.object({
  ...
  // 가격은 입력받지 않음 — 서버에서 DB 재조회 (PART 14-1 연동)
});
...
sanitize-html (Node.js)     UGC HTML 서버사이드 새니타이징 (PART 5-3)       Node.js 14+ / XSS 방어 필수
flutter_html                Flutter HTML 렌더링 (태그 필터링 가능)           Flutter 3.0+ / XSS 주의 PART 5-3
http_certificate_pinning    SSL Pinning — SPKI 해시 비교 (PART 4-1)        Flutter 3.0+ / 네이티브 코드 기반
package:cronet_http         HTTP/3(QUIC) 지원 HTTP 클라이언트 (PART 4-4)    Flutter 3.0+ / Android only (iOS는 URLSession 자동)
                            ⚠️ iOS는 별도 불필요 — URLSession이 HTTP/3 자동 협상
[PQC 패키지 — 2026 현재]:   Dart 공식 PQC 패키지 미출시. PART 4-5 참조.     Flutter SDK 공식 지원 후 채택 예정
...
SECURITY_CHANGELOG.md 작성 포맷

> 상세 변경 이력은 별도 SECURITY_CHANGELOG.md 파일에 기록한다.
> 아래 포맷에 맞춰 작성한다.

```markdown
# SECURITY_MASTER 변경 이력
...
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ① | 예: iOS 백그라운드 스냅샷 보안 | 6-3 | iOS 앱 스위처 민감 화면 노출 방지 |

### 변경 (CHANGED)
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ② | 예: CF Gen1 문법 기준 명시 | 7 전반 | Gen1/Gen2 혼용 방지 |

### 보강 (ENHANCED)
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ③ | 예: split-debug-info 보관 정책 | 8-2 | 크래시 심볼리케이션 보장 |

### 구조 (STRUCTURE)
| # | 변경 | 영향 | 사유 |
|---|------|------|------|
| ④ | 예: TOC 추가 | 파일 상단 | 네비게이션 개선 |
```

**작성 규칙**
```
✅ 모든 변경에 영향 PART 번호 명시 — AI가 관련 섹션만 재확인 가능
✅ 변경 유형 4가지: NEW(신규) / CHANGED(변경) / ENHANCED(보강) / STRUCTURE(구조)
✅ 사유는 1줄 요약 — "왜 변경했는지"만 기재
✅ 버전별 날짜 필수 기재
❌ 코드 전문 포함 금지 — 변경 사항 요약만 (코드는 본문 참조)
```

---


*출처: OWASP Foundation (Mobile Top 10 2024, MASVS v2.0, MASTG, JWT Cheat Sheet, LLM Top 10 2025,
API Security Top 10, Session Management Cheat Sheet, Authentication Cheat Sheet, Logging Cheat Sheet) /
NIST SP 800-57 · SP 800-63B (Rev.4 포함) · SP 800-92 · SP 800-175B · AI RMF · IR 8547 (PQC 전환) /
NIST FIPS 203 (ML-KEM) · FIPS 204 (ML-DSA) · FIPS 205 (SLH-DSA) /
NSA CNSA 2.0 / RFC 7636 (PKCE) / IETF RFC 9000 (QUIC) · RFC 9001 (QUIC-TLS) · RFC 9114 (HTTP/3) /
W3C WebAuthn Level 3 / FIDO Alliance (Passkeys) /
Apple Keychain Services · ATS · Local Authentication · App Store Review Guidelines /
Apple App Privacy · SDK Privacy Manifests · Sign in with Apple · Passkeys · WWDC23/24 /
Google Android Security (Keystore, BiometricPrompt, network_security_config, App Links, Credential Manager) /
Google Play Data Safety · Play Integrity API · Data Deletion Policy 2024 /
...
flutter_secure_storage · freerasp (Talsec) · flutter_appauth · sign_in_with_apple /
@firebase/rules-unit-testing · Firebase Cloud Functions Security · CF Gen1→Gen2 Migration /
...
Anthropic Usage Policy · OpenAI API Security /
Zod (Schema Validation) /
NCMEC CyberTipline (CSAM 신고) /
GDPR(EU 2016/679) · PIPA(개인정보보호법, 한국) · US REPORT Act 2024 · EU DSA*

---

## ▌SECURITY_MASTER v1.2 변경 이력
...
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ① | 파일 헤더 선언 — PART 1~12 범용 / PART 13 프로젝트별 작성 범위 명확화 | 파일 상단 | "범용 파일" 선언과 PART 13 특화 예시 내용 간 충돌 해소 |
| ② | PART 13 섹션 헤더 — 신규 프로젝트 교체 필수 안내 문구 추가 | 13 전체 | AI가 예시 내용을 그대로 적용하는 오작동 방지 |
| ③ | PART 13-6-2 AI API 예시 모델명 업데이트 | 13-6 | claude-sonnet-4-20250514 → claude-sonnet-4-6 (현행 최신 모델) |
```

---

## ▌SECURITY_MASTER v1.1 변경 이력

> 아래는 v1.0 → v1.1 변경 사항을 SECURITY_CHANGELOG.md 포맷에 맞게 기록한 것이다.
> 상세 변경 이력은 별도 SECURITY_CHANGELOG.md로 분리 관리할 수 있다.
...
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ① | HTTP/3 & QUIC 보안 — 0-RTT Replay 방어, Pinning 호환성, 테스트 주의사항 | 4-4 | Flutter Firebase 앱에서 HTTP/3 자동 협상 증가 → 보안 이해 필요 |
| ② | 포스트-양자 암호화(PQC) Crypto-Agility 준비 — NIST FIPS 203/204/205 대응 | 4-5 | NIST 2024.08 표준 확정, HNDL 위협, Crypto-Agility 코드 구조 필수화 |
| ③ | PART 13 구체적 예시 채우기 — 현장 진단 B2B 앱 기준 | 13-0~13-4 | AI 작업 정밀도 향상을 위해 빈 템플릿 → 실질 참조 예시로 전환 |

### 변경 (CHANGED)
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ④ | Phase 4 분기 지침에 PQC Crypto-Agility 항목 추가 | 헤더 Phase 분기 | Phase 4+ 에서 PQC 준비 확인 강제화 |
| ⑤ | PART 10 네트워크 체크리스트에 Idempotency-Key, AES-256 확인, Crypto-Agility 항목 추가 | 10 | HTTP/3 및 PQC 실무 체크 반영 |
| ⑥ | 부록 패키지 목록에 cronet_http, PQC 패키지 현황 추가 | 부록 | HTTP/3 Flutter 대응 경로 명확화 |

### 보강 (ENHANCED)
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ⑦ | PART 13-1 발행 후 수정 차단 — Firestore Rules 구체적 코드로 보강 | 13-1 | 템플릿 수준에서 실동작 Rules로 격상 |
| ⑧ | PART 13-3 사진 업로드 GPS 제거 — 실제 flutter_exif 구현 코드 추가 | 13-3 | 코드 없는 지침 → 바로 쓸 수 있는 구현체로 보강 |
| ⑨ | PART 13-4 오프라인 보안 — Hive AES-256 초기화 + 동기화 충돌 정책 코드화 | 13-4 | 빈 템플릿 → 실동작 Flutter 코드로 보강 |

### 구조 (STRUCTURE)
| # | 변경 | 영향 | 사유 |
|---|------|------|------|
| ⑩ | TOC PART 4 항목에 4-4/4-5 추가 | 파일 상단 TOC | 신규 섹션 탐색 지원 |
| ⑪ | 보안 레이어 구조도 PART 4 설명 업데이트 | 파일 상단 | HTTP/3 + PQC 커버리지 반영 |
| ⑫ | 참조 표준 문서에 IETF RFC 9000/9001/9114, NIST FIPS 203/204/205, NSA CNSA 2.0 추가 | 부록 | 신규 PART 4-4/4-5 근거 문서 추가 |
| ⑬ | Source of Truth 파일명 v1.0 → v1.1 업데이트 | 파일 상단 | 버전 명확화 |
```
...
## ▌SECURITY_MASTER v2.0 변경 이력

> v1.2 → v2.0 변경 사항 (2026-03-22)
> 2026 글로벌 사이버보안 딥리서치 기반 대규모 업그레이드
...
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ① | §0 상호참조 맵 — 3파일 체계 연결 테이블 | 헤더 | 파일 간 참조 관계 명확화, 읽는 순서 정의 |
| ② | PART 14 결제·구독 보안 — 범용 결제 원칙 독립 | 14 신규 | 기존 13-5 IAP 예시를 범용 원칙으로 격상 |
| ③ | PART 15 AI 에이전트 보안 — OWASP Agentic AI Top 10 (2026) | 15 신규 | 보안 전문가 48%가 2026 최대 위협 지목 |
| ④ | PART 16 인시던트 대응 — 5단계 플로우 + 포스트모템 | 16 신규 | PART 12-4 모니터링과 연계한 대응 체계 |
| ⑤ | PART 17 규제 매핑 — EU AI Act, NIS2, DORA, CMMC 2.0 | 17 신규 | 2026 신규 규제 + 기존 GDPR/PIPA 통합 |
| ⑥ | PART 18 Supabase 보안 — RLS, Auth, Edge Functions | 18 신규 | Firebase 대안 보안 규칙 체계 |
| ⑦ | 8-6 SBOM — 소프트웨어 자재명세서 | 8-6 신규 | 공급망 공격 3배 증가 대응, CI/CD 통합 |
| ⑧ | 11-5 HNDL 대응 — 양자 위협 데이터 보호 | 11-5 신규 | Harvest Now Decrypt Later 실질 대응 |
| ⑨ | Phase 게이트 — Phase 1~4+ 보안 요구사항 | 10 확장 | CLAUDE.md Phase와 1:1 연동 체크리스트 |
| ⑩ | 용어 정의 테이블 — NHI, MCP, SBOM 등 14개 | 부록 | 2026 신규 보안 용어 정의 |

### 변경 (CHANGED)
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ⑪ | 파일 버전 v1.2 → v2.0 | 전체 | 대규모 업그레이드 반영 |
| ⑫ | 헤더 Phase 분기에 PART 14~18 참조 추가 | 헤더 | 신규 PART 네비게이션 |
| ⑬ | TOC에 PART 14~18 + §0 추가 | 헤더 | 완전한 목차 |
| ⑭ | 보안 레이어 구조도에 PART 14~18 추가 | 헤더 | 레이어 완전성 |
| ⑮ | 준거 표준에 OWASP Agentic AI, CrowdStrike, Gartner, EU AI Act 추가 | 헤더 | 2026 준거 근거 |
| ⑯ | Source of Truth 선언 — [필수]/[금지]/[권장] 강도 체계 추가 | 헤더 | v5 규칙 강도 체계 병합 |

### 구조 (STRUCTURE)
| # | 변경 | 영향 | 사유 |
|---|------|------|------|
| ⑰ | PART 1~13 원문 100% 보존 | 1~13 | 기존 코드·규칙 무손상 보장 |
| ⑱ | PART 14~18을 13 이후·부록 이전에 배치 | 전체 구조 | 기존→신규 자연스러운 흐름 |
| ⑲ | 부록에 용어 정의 테이블 삽입 | 부록 | 2026 신규 용어 빠른 참조 |
```
...
## ▌SECURITY_MASTER v2.1 변경 이력

> v2.0 → v2.1 변경 사항 (2026-03-22)
> MASWE 매핑 통합 + NIST SSDF·EU CRA 신규 규제 반영 + AI 위협 심화 섹션 추가
...
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ① | 15-7 AI 기반 취약점 발견의 무기화 — Schneier·Anthropic 2026 연구 반영 | 15 신규 | AI가 OpenSSL 12개 제로데이 발견, 익스플로잇 속도 가속 대응 |
| ② | 15-8 Vibe Coding / AI 코딩 보안 위험 — Gartner 2026 Trend 1 반영 | 15 신규 | AI 생성 코드 보안 리뷰 미적용 프로덕션 배포 위험 급증 대응 |
| ③ | PART 1 MASWE 연동 안내 — MASVS v2.0 이후 재편된 약점 열거 체계 | 1 신규 | MASWE-XXXX 단위 테스트 추적성 확보, PART별 MASVS 그룹 매핑 |
| ④ | 17-1 규제 매핑 — NIST SSDF v1.2 (SP 800-218) 추가 | 17 확장 | US EO 14028 연방 필수, Secure SDLC 4단계 요구사항 |
| ⑤ | 17-1 규제 매핑 — EU CRA (Cyber Resilience Act) 추가 | 17 확장 | SBOM 필수·취약점 보고 의무·CE 마킹 연동 |
| ⑥ | PART 6-5 RASP — 디버거·후킹·변조·에뮬레이터 감지 통합 | 6 신규 | PART 10 체크리스트 RASP 항목 연동 |
| ⑦ | PART 7-7-1 Firebase 노출 방어 — 'if true' 자동 검증, google-services.json 서명 제한 | 7 신규 | 730TB+ 노출 사례(Cybernews 2026) 재발 방지 |

### 변경 (CHANGED)
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ⑧ | 파일 버전 v2.0 → v2.1 | 전체 | 신규 섹션 반영 버전 업 |
| ⑨ | 준거 표준에 NIST SSDF v1.2, OWASP MASWE, EU CRA 추가 | 헤더 | v2.1 신규 PART 근거 문서 |

### 보강 (ENHANCED)
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ⑩ | PART 2 데이터 보안 통계 보강 — Firebase 730TB+ 노출, 모바일 침해 비용 $6.99M | 2 | 경영진 설득 데이터 최신화 (Cybernews 2026.02) |
| ⑪ | PART 10 체크리스트 — RASP·Firebase 노출 방어·AI 코딩 보안 항목 추가 | 10 | 신규 PART 6-5·7-7-1·15-7·15-8 체크리스트 반영 |
| ⑫ | PART 11-5 HNDL 대응 신설 — 양자 위협 데이터 보호 전략 | 11 | Harvest Now Decrypt Later 실질 대응 절차 추가 |
| ⑬ | PART 8-6 SBOM 신설 — CycloneDX 포맷 공급망 자재명세서 | 8 | 공급망 공격 3배 증가 대응 + EU CRA 의무화 선제 준비 |

### 구조 (STRUCTURE)
| # | 변경 | 영향 | 사유 |
|---|------|------|------|
| ⑭ | TOC PART 8·11 항목에 8-6 SBOM·11-5 HNDL 추가 | 헤더 | 신규 섹션 탐색 지원 |
| ⑮ | 부록 용어 정의에 DaaS·DORA·NIS2·CMMC·CRA 추가 | 부록 | v2.1 신규 규제·용어 빠른 참조 |
| ⑯ | Bug Fix — PART 2-2 코드 블록 내 부록 URL 오염 제거 | 2-2 | AI 코드 파싱 오류 방지 (Dart 문법 위반 URL 라인 제거) |
| ⑰ | Bug Fix — 부록 빈 헤더 (###) 제거 | 부록 | 마크다운 렌더링 정리 |
```
...
## ▌SECURITY_MASTER v2.2 변경 이력

> v2.1 → v2.2 변경 사항 (2026-03-22)
> 공격·복제 대비 공백 보완 + 웹앱 보안 신설 + 구조 버그 완전 수정
...
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ① | 6-6 문자열·상수 보호 — envied obfuscate, ADB backup 차단, 내부 URL 보호 | 6 신규 | 역공학으로 내부 엔드포인트·테스트 자격증명 노출 방어 |
| ② | 6-7 물리 기기 탈취 대응 — 원격 세션 무효화 CF, USB 감지 블러, revokeRefreshTokens | 6 신규 | 기기 분실·도난 시 피해 최소화 절차 코드화 |
| ③ | 7-9 App Check 서버사이드 검증 — verifyToken, enforceAppCheck 패턴 | 7 신규 | 클라이언트 활성화만으로 클론 앱 차단 불가 → 서버 검증 필수 |
| ④ | 7-10 FCM 푸시 알림 보안 — 페이로드 원칙, 토큰 관리, Silent Push 방어 | 7 신규 | FCM 페이로드 민감 데이터 포함 + 토큰 탈취 패턴 차단 |
| ⑤ | 7-11 Race Condition & 비즈니스 로직 — Transaction 패턴, Idempotency-Key, 선착순 | 7 신규 | 쿠폰·재고·선착순 이벤트에서 실제 가장 많이 발생하는 공격 방어 |
| ⑥ | 12-5 계정 탈취 사후 대응 — 새 기기 감지 CF, 강제 로그아웃 신호, 자동 잠금 | 12 신규 | 탈취 성공 이후 시나리오 방어 + 조기 탐지 체계 |
| ⑦ | PART 19 Next.js·웹앱 보안 — 6개 섹션 (CSP, CSRF, 헤더, 환경변수, XSS/리다이렉트, API Route) | 19 신규 | Flutter 파일이지만 Next.js 프로젝트(CryoNode 등)에 적용할 웹 전용 보안 규칙 전무 → 보완 |

### 변경 (CHANGED)
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ⑧ | 파일 버전 v2.1 → v2.2 | 전체 | 신규 섹션 반영 버전 업 |
| ⑨ | Phase 분기 — Phase 2에 PART 19, Phase 3에 7-9·7-11·12-5, Phase 4에 6-6·6-7 추가 | 헤더 | 신규 PART 효율적 참조를 위한 분기 업데이트 |
| ⑩ | 보안 레이어 구조도 — PART 6·7·12·19 설명 업데이트 | 헤더 | 신규 섹션 커버리지 반영 |
| ⑪ | 준거 표준 — OWASP API Security Top 10, W3C CSP Level 3 추가 | 헤더 | PART 7-11·19 근거 문서 |

### 보강 (ENHANCED)
| # | 변경 | 영향 PART | 사유 |
|---|------|-----------|------|
| ⑫ | PART 10 체크리스트 — 물리보안·클론앱방어·Race Condition·웹앱보안 4개 카테고리 추가 | 10 | 신규 PART 6-6·6-7·7-9~7-11·19 배포 전 검증 항목화 |
| ⑬ | TOC — PART 6·7·12·19 섹션 번호 및 신규 섹션 반영 | 헤더 | 완전한 목차 |

### 구조 (STRUCTURE)
| # | 변경 | 영향 | 사유 |
|---|------|------|------|
| ⑭ | Bug Fix A — 6-4/6-5 사이 이중 구분선(--- 2개) 제거 | 6 | AI 파싱 시 빈 섹션 오판 방지 |
| ⑮ | Bug Fix B — 7-7/7-7-1 사이 이중 구분선(--- 2개) 제거 | 7 | AI 파싱 시 빈 섹션 오판 방지 |
```
