# DESIGN MASTER KNOWLEDGE BASE v3.2
...
# v3.1 → v3.2: ① PART 12-5 PrimaryButton — widget.isLoading 버그 수정
#               ② PART 12-5 PrimaryButton — height 52 → 72 (CTA 72dp 기준 적용)
#               ③ PART 12-1 AppColors — darkOnAccent / lightOnAccent 토큰 추가
#               ④ PART 12-2 AppTheme — onPrimary AppColors.on-accent 토큰으로 교체
#               ⑤ PART 12-5 PrimaryButton — 텍스트/로딩 컬러 on-accent 토큰 연결
#               ⑥ PART 3-7 체크리스트 항목 6 — 터치 기준 CLAUDE.md 테이블 참조로 수정
#               ⑦ PART 12-4 StatusBadge — _resolveColors 토큰 교체 패턴 안내 강화
#               ⑧ PART 12-7 EmptyState 위젯 신규 추가 (Krug 3요소 구현체)
#               ⑨ PART 12-8 ErrorState 위젯 신규 추가 (Nielsen 3상태 구현체)
#               ⑩ PART 3-6 터치 타겟 예시 — Icons.close → LucideIcons.x 교체
#               ⑪ PART 5-3 도메인/분위기 필드 CLAUDE.md 연동 안내 명확화
#               ⑫ 출처 섹션 — Don Norman 누락 추가
...
SizedBox(
  ...
  child: IconButton(
    iconSize: 24,
    onPressed: () {},
    icon: const Icon(LucideIcons.x), // Lucide Icons 사용 (lucide_flutter 패키지)
  ),
)
```
...
[ ] 6. 터치 타겟이 CLAUDE.md 기준을 충족하는가?
       일반 44dp / 전문직·장갑 56dp / 리스트 48dp / 바텀 탭 64dp / CTA 72dp
       → "특이사항 > 터치 기준" 테이블이 유일한 기준 (Wroblewski)
...
**프로젝트 personality 정의 (CLAUDE.md "기본 정보" + "디자인 시스템" 섹션에서 확인)**
```
// ★ 아래 두 항목은 CLAUDE.md "디자인 시스템" 섹션 최상단에 함께 기재할 것
// 비어있으면 작성 요청 후 컬러 토큰 작성으로 진행
...
> ★ **역할 기반 IA 구현(Firestore Rules + 역할 분기)** → SECURITY_MASTER_v2.2.md PART 12 참조
> IA 설계(여기)는 어떤 역할이 어떤 탭을 보는지 정의하고,
> PART 12는 그 역할 기반 접근 제어를 Firestore Rules로 강제하는 방법을 정의한다.
...
인터넷 끊김 감지 즉시:
  ...
  → 서브텍스트: "주요 기능은 오프라인에서도 사용 가능" (← 프로젝트별 문구 교체)
...
```
백그라운드 업데이트 완료 시:
  → LEVEL 3 INFO 알림: "[DB 이름] 업데이트됨 (최신 버전 적용)"
  → 탭 시 변경 내용 요약 바텀시트
...
| 항목 카드 상태   | 레드 테두리 글로우    | 앰버 테두리            | 변화 없음       |
...
3. 항목 카드: border-color fade-in 0.5s + glow pulse 반복 (해제 전까지)
...
```
중복 알람 억제:
  동일 항목 + 동일 원인 → 30분 내 재발생 시 묶어서 1개로 표시
  카드에 "(3회 발생)" 카운터 표시
...
- [ ] `[RBAC]` Firestore Rules에 역할 기반 접근 제어 구현됐는가 → SECURITY_MASTER_v2.2.md PART 12
...
Luke Wroblewski / Don Norman(The Design of Everyday Things) /
Apple HIG + iOS 26 Liquid Glass + ProMotion /
...
class AppColors {
  ...
  static const darkOnAccent    = Color(0xFF______); // --on-accent (CTA 버튼 위 텍스트/아이콘 색)
  ...
  static const lightOnAccent   = Color(0xFF______); // --on-accent (CTA 버튼 위 텍스트/아이콘 색)
...
class AppTheme {
  static ThemeData get dark => ThemeData(
    ...
    colorScheme: const ColorScheme.dark(
      ...
      onPrimary: AppColors.darkOnAccent,  // --on-accent (CTA 버튼 위 텍스트)
  ...
  static ThemeData get light => ThemeData(
    ...
    colorScheme: const ColorScheme.light(
      ...
      onPrimary: AppColors.lightOnAccent, // --on-accent (CTA 버튼 위 텍스트)
...
// ⚠️ _resolveColors 내 Color(0x1F..) 등 하드코딩 값은 레퍼런스 패턴 전용이다.
// 실제 구현 시 AppColors에 아래 배지 전용 토큰을 추가 후 교체할 것.
//
// CLAUDE.md AppColors에 추가할 토큰 예시:
//   static const darkSuccessBg     = Color(0x1F34D399); // success.withOpacity(0.12)
//   static const darkSuccessBorder = Color(0x4034D399); // success.withOpacity(0.25)
//   (darkWarning/darkDanger/darkInfo 계열도 동일 패턴으로 추가)
//
    ...
    return GestureDetector(
      ...
      child: AnimatedScale(
        ...
        child: Container(
          ...
          height: 72, // CTA 버튼 최소 기준 — CLAUDE.md "터치 기준" 테이블
          decoration: BoxDecoration(
          ...
          child: widget.isLoading
              ? SizedBox(
                  ...
                  child: CircularProgressIndicator(
                    strokeWidth: 2,
                    // ★ CTA 위 컬러: CLAUDE.md --on-accent 토큰 사용
                    // 토큰 미정의 시 임시 폴백: 다크=darkBg, 라이트=lightSurface
...
// PART 3-1 Krug Empty State 3요소 (아이콘 + 제목 + CTA) 구현체
//
// 사용 예:
//   EmptyState(
//     icon: LucideIcons.folderOpen,
//     title: context.l10n.emptyStateTitle,
//     description: context.l10n.emptyStateDescription, // 선택
//     actionLabel: context.l10n.emptyStateAction,       // 선택
//     onAction: () => ...,                              // 선택
//   )
// ⚠️ 모든 텍스트는 context.l10n.키명 사용 — 문자열 직접 전달 금지
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            // 아이콘 — 래퍼 박스 없이 직접 배치 (PART 1-4 원칙)
            Icon(
              icon,
              size: 56,
            ...
            if (actionLabel != null && onAction != null) ...[
              ...
              // 엄지존 아님 — 빈 상태 CTA는 콘텐츠 중앙에 인라인 배치
              OutlinedButton(
                onPressed: onAction,
                style: OutlinedButton.styleFrom(
                  minimumSize: const Size(0, 44), // 최소 터치 타겟 준수
                  side: BorderSide(
...
// PART 3-3 Nielsen 3상태 中 Error 구현체
// 에러 메시지는 인간 언어로 — 기술 코드 금지 (Nielsen 휴리스틱 #9)
//
// 사용 예:
//   ErrorState(
//     message: context.l10n.errorNetworkMessage,
//     onRetry: () => ref.refresh(myProvider),
//   )
// ⚠️ message는 context.l10n.키명 사용 — 문자열 직접 전달 금지

class ErrorState extends StatelessWidget {
  final String message;
  final VoidCallback? onRetry;
  final String? retryLabel; // 기본값: commonRetry arb 키
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            // 에러 아이콘 — 래퍼 박스 없이 직접 배치 (PART 1-4 원칙)
            Icon(
            ...
            if (onRetry != null) ...[
              ...
              FilledButton.icon(
                ...
                style: FilledButton.styleFrom(
                  ...
                  foregroundColor: isDark ? AppColors.darkOnAccent : AppColors.lightOnAccent,
                ),
...
✅ 수치는 항상 Monospace 폰트
✅ 터치 타겟: 기본 44px / 전문직·장갑 환경 56px / 바텀 탭 64dp / CTA 72dp (CLAUDE.md 기준 확인)
✅ 버튼은 항상 엄지존(하단)
✅ 로딩/성공/에러 3상태 항상 설계 — EmptyState/ErrorState 위젯 PART 12-7·12-8 사용
